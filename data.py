...
def load_dataset_seq(path):
    m_item = 0
    m_item_, all_train_ind = load_file(path + "/train_after_final.dat")
    m_item = max(m_item, m_item_)
    m_item_, all_test_ind = load_file(path + "/test_after_final.dat")
    m_item = max(m_item, m_item_)

    items_popularity = np.zeros(m_item + 1)
    items_popularity[0] = np.inf
    for items in all_train_ind:
        for item in items:
            items_popularity[item] += 1
    # for items in all_test_ind:
